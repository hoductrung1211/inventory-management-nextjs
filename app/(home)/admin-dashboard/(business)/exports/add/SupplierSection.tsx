    ...
    return (
        <main className="flex flex-col h-full gap-5">
            <div className="flex-shrink-0 flex justify-center h-10  font-bold">
                <button 
                    className={ "rounded-l-md " + (customerMode ? btn : activeBtn)}
                    onClick={handleChangeMode}
                >
                    Select Customer
                </button>
                <button 
                    className={"rounded-r-md " + (customerMode ? activeBtn : btn)}
                    onClick={handleChangeMode}
                >
                    Create Information
                </button>
...
export function SelectCustomer({
    ...
    customerId?: number,
    handleChangeCustomer: (id: number) => void
}) {
    ...
    const filterdCustomers = customers?.filter(p => {
        const check = 
    ...
    async function fetchCustomers() {
        try {
            ...
            if (data?.[0] && customerId == undefined) 
                handleChangeCustomer(data?.[0].id);
        }
        catch(error) {
            console.log(error);
        }
    ...
    function handleClickCustomer(id: number) {
        const newCustomer = customers?.find(p => p.id === id);

        if (newCustomer)
            handleChangeCustomer(newCustomer.id);
    }
    ...
    return (
        <section className="flex flex-col gap-3 h-full max-h-[560px] px-3 pt-5 rounded-md bg-gray-50 bg-opacity-70 overflow-auto">
            ...
            <section className="h-full max-h-[420px] bg-white">
                <Table
                    handleRowClick={handleClickCustomer}
                    columns={[
                        {id: 1, text: "Id", key: "id"},
